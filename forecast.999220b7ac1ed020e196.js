...
function getMinTemp(weatherData) {
  var minTemp = weatherData.hourly.reduce(function (min, current) {
    return Math.min(min, current.temp);
  }, weatherData.hourly[0].temp);
  return Math.round(minTemp);
}
function getMaxTemp(weatherData) {
  var maxTemp = weatherData.hourly.reduce(function (max, current) {
    return Math.max(max, current.temp);
  }, weatherData.hourly[0].temp);
  return Math.round(maxTemp);
}
function createDescriptionString(weatherData) {
  return "".concat(getMinTemp(weatherData), "\xB0c / ").concat(getMaxTemp(weatherData), "\xB0c, ").concat(weatherData.current.weather[0].main);
}
function setWeatherData(weatherData) {
  location.textContent = "".concat(weatherData.name, ", ").concat(weatherData.country);
  date.textContent = "".concat(getCurrentDate());
